<template>
  <BaseLayout>
    <div class="px-[100px] mt-[100px] flex- flex-col bg-[#F7F8FA]">
      ...
      <div class="mt-[40px] flex flex-row justify-between">
            ...
            <div class="w-full mt-[40px] flex flex-wrap gap-[20px]">
              ...
              >
                ...
                <div class="w-full flex flex-row">
                  <div
                    class="w-[46px] h-[46px] mr-[14px] flex items-center justify-center"
                  >
                    <img :src="item.icon" alt="" />
                  </div>
                  <div class="w-full">
                    <p
                      class="text-[16px] text-[#868689] leading-[24px] font-medium"
                    >
                      {{ item.title }}
                    </p>
                    <p
                      class="text-[18px] text-[#1E1F24] leading-[24px] font-medium mt-[6px]"
                    >
                      {{ item.description }}
                    </p>
                  </div>
        ...
        <div class="flex flex-col w-[30%]">
          ...
          <div
            @click="openModalGetAQuote = true"
            class="flex flex-row items-center justify-center py-[14px] w-[224px] mt-[50px] rounded-[8px] bg-[#0066FF]"
          >
            ...
            <UIcon
              ...
              class="ml-[5px] text-white text-[16px]"
            />
          </div>
        </div>
      </div>
    ...
    <UModal
      ...
      v-model="openModalGetAQuote"
      :transition="false"
    >
      <div class="px-[36px] py-[22px] flex flex-col">
        <div class="w-full flex justify-end">
          <div
            @click="openModalGetAQuote = false"
...
async function onSubmit() {
  try {
    ...
    // example
    //      const response = await $axiosStrapi.post(
    //   '/api/application/submitExcel',
    //   {
    //     email: state.email,
    //     phone_number: state.phone_number,
    //     full_name: state.full_name,
    //     title: props.title,
    //     cv_file: fileUrl?.value ? $fullStrapiUrl(fileUrl.value) : '',
    //   },
    //   {
    //     header: {
    ...
    //   // Reset lại form sau khi gửi thành công (tùy chọn)
    //   state.email = ''
    //   state.phone_number = ''
    //   state.full_name = ''
    // }
